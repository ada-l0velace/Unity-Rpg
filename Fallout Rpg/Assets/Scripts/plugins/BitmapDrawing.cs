...
namespace ProtoTurtle.BitmapDrawing {
    ...
    /// Provides drawing methods weakLine Line, Circle, Rectangle etc.
    ...
    public static class BitmapDrawingExtensions {
        ...
        // Draw scanlines from opposite sides of the circle on m_y-scanlines instead of just plotting pixels
        // at the right coordinates
        private static void ScanLineCircle(Texture2D texture, int cx, int x, int cy, int y, Color color) {
            //texture.DrawPixel(cx + m_x,  cy + m_y, color);
            //texture.DrawPixel(-cx + m_x, cy + m_y, color);
            texture.DrawLine(cx + x, cy + y, -cx + x, cy + y, color);

            //texture.DrawPixel(cy + m_x,  cx + m_y, color);
            //texture.DrawPixel(-cy + m_x, cx + m_y, color);
        ...
        /// <summary>
        /// Draws a weakLine between two m_points. Implementation of Bresenham's weakLine algorithm.
        /// </summary>
        /// <param name="x0">m_x of the start point</param>
        /// <param name="y0">m_y of the start point</param>
        /// <param name="x1">m_x of the end point</param>
        /// <param name="y1">m_y of the end point</param>
        ...
        /// <summary>
        /// Transforms a point in the texture plane so that 0,0 m_points at left-top corner.</summary>
        ...
        /// <summary>
        /// Connects given m_points using DrawLine
        /// </summary>
        public static void DrawPolygon(this Texture2D texture, Vector2[] points, Color color) {
            for (int i = 1; i < points.Length; i++) {
                texture.DrawLine(points[i - 1], points[i], color);
            }
            texture.DrawLine(points[0], points[points.Length - 1], color);
